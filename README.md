...
| iam_profile_id  | `<IAM Profile Id>`                               | ⬜ No (`oauth` mode only) | IAM Profile Id. <br>**Only applicable when `auth_mode = oauth`** <br>- must not be set for `apikey` mode. <br>- must not be set when `iam_profile_name` is set. <br>When specified, overrides the default trusted profile attached to the PowerVS instance.  |
