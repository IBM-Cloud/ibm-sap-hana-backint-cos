...
4. Optionally set **`iam_profile_id`** or **`iam_profile_name`** to select a specific Trusted Profile other than the one attached to the instance by default. Set at most one of the two — specifying both is a configuration error.
...
| iam_profile_id  | `<IAM Profile Id>`                               | ⬜ No (`oauth` mode only) | IAM Profile Id. <br>**Only applicable when `auth_mode = oauth`** <br>- must not be set for `apikey` mode. <br>- must not be set when `iam_profile_name` is set. <br>- must being with **Profile-xxx**. <br>- must be specified **without** `double quotes ""` . <br>When specified, overrides the default trusted profile attached to the PowerVS instance.  |
| iam_profile_name  | `<IAM Profile Name>`                               | ⬜ No (`oauth` mode only) | IAM Profile Name. <br>**Only applicable when `auth_mode = oauth`** <br>- must not be set for `apikey` mode. <br>- must not be set when `iam_profile_id` is set. <br>When specified, overrides the default trusted profile attached to the PowerVS instance. |
